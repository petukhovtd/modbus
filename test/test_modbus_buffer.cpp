...
TEST(ModbusBuffer, ConvertToType) {
    ...
    static const AduBuffer asciiReference = {asciiStart, '0', '1', '0', '3', '0', '4', 'D', '8', asciiEnd >> 8u,
                                             asciiEnd & 0xFFu};

    {
        ...
        EXPECT_TRUE(
                test::Compare(tcpReference.begin(), tcpReference.end(), modbusBuffer.cbegin(), modbusBuffer.cend()));
    }
    {
        ModbusBuffer modbusBuffer = test::MakeModbusBuffer(rtuReference, FrameType::RTU);
        ...
        EXPECT_TRUE(test::Compare(asciiReference.begin(), asciiReference.end(), modbusBuffer.cbegin(),
                                  modbusBuffer.cend()));
    }
    {
        ...
        EXPECT_TRUE(
                test::Compare(rtuReference.begin(), rtuReference.end(), modbusBuffer.cbegin(), modbusBuffer.cend()));
    }
    {
        ModbusBuffer modbusBuffer = test::MakeModbusBuffer(tcpReference, FrameType::TCP);
